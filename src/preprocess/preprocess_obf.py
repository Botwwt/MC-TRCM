...
DATASET_ID = "obf"
RAW_BASE = PROJECT_ROOT / "data_raw" / DATASET_ID / "extracted" / "OBF-Psychiatric Dataset"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build canonical OBF-Psychiatric tables.")
...
def compute_window_feature_bundle(window_daily: pd.DataFrame, full_daily: pd.DataFrame) -> dict[str, float | None]:
...
def build_window_row(subject_id: str, anchor_id: str, task_name: str, anchor_date: pd.Timestamp, daily_frame: pd.DataFrame, static_row: pd.Series) -> dict:
    ...
    symptom_value = safe_float(static_row.get("madrs")) if "madrs" in static_row.index else safe_float(static_row.get("madrs1"))
    row["modality_mask_symptom_context"] = 1 if symptom_value is not None else 0
    ...
    concept_mask_map = {
        ...
        "c6": 0,
        "c7": 0,
        "c8": 1 if symptom_value is not None else 0,
    }
    ...
    row["feat_static_baseline_severity"] = symptom_value
    row["feat_static_missing_rate"] = float(static_row.isna().mean())

    row["feat_symptom_context_mean_short"] = symptom_value
    row["feat_symptom_context_mean_medium"] = symptom_value
    row["feat_symptom_context_mean_long"] = symptom_value
    row["feat_symptom_context_missing_ratio_short"] = 0.0 if symptom_value is not None else 1.0
    row["feat_symptom_context_missing_ratio_medium"] = 0.0 if symptom_value is not None else 1.0
    row["feat_symptom_context_missing_ratio_long"] = 0.0 if symptom_value is not None else 1.0
...
def load_group_info() -> tuple[pd.DataFrame, dict[str, str]]:
    group_files = {
        "control": "control-info.csv",
        "clinical": "clinical-info.csv",
        "depression": "depression-info.csv",
        "schizophrenia": "schizophrenia-info.csv",
        "adhd": "adhd-info.csv",
    }
    frames = []
    number_to_group = {}
    for group_name, filename in group_files.items():
        frame = pd.read_csv(RAW_BASE / filename)
        frame["group_label"] = group_name
        frames.append(frame)
        for subject_id in frame["number"]:
            number_to_group[subject_id] = group_name
    return pd.concat(frames, ignore_index=True, sort=False), number_to_group
...
def main() -> int:
    ...
    info_df, number_to_group = load_group_info()
    subject_files = []
    for group_name in ["control", "clinical", "depression", "schizophrenia", "adhd"]:
        subject_files.extend(sorted((RAW_BASE / group_name).glob("*.csv")))
    ...
    for _, static_row in info_df.iterrows():
        subject_id = static_row["number"]
        ...
        group_label = number_to_group[subject_id]
        ...
        participants_rows.append(
            {
                ...
                "sex": static_row.get("gender"),
                "age": static_row.get("age"),
                "baseline_group": group_label,
                "baseline_severity": symptom_value,
                "static_missing_rate": float(static_row.isna().mean()),
                "source_file": str((RAW_BASE / group_label / f"{subject_id}.csv").relative_to(PROJECT_ROOT)),
            ...
            daily_rows.append(
                {
                    ...
                    "raw_symptom_value": symptom_value,
        ...
        def append_anchor(task_name: str, label_type: str, y_raw, class_label):
            ...
            window_rows.append(build_window_row(subject_id, anchor_id, task_name, anchor_date, daily, static_row))

        append_anchor("obf_5class", "multiclass", group_label, group_label)
        if group_label in {"control", "depression"}:
            append_anchor("dep_binary", "binary", 1 if group_label == "depression" else 0, group_label)
        if group_label in {"control", "clinical"}:
            append_anchor("clinical_vs_control", "binary", 1 if group_label == "clinical" else 0, group_label)
    ...
    audit_payload = {
        ...
        "dataset_id": DATASET_ID,
        "raw_subjects_in_metadata": int(len(info_df)),
        ...
        "task_definitions": {
            "dep_binary": "depression vs control only",
            "clinical_vs_control": "clinical vs control only",
            "obf_5class": "control / clinical / depression / schizophrenia / adhd",
        },
        "assumptions": [
            "End-of-recording timestamp is used as the anchor for all first-pass OBF tasks.",
            "Daily activity summaries are built from winsorized minute-level actigraphy.",
        ],
    }
    write_json(PROJECT_ROOT / "outputs" / "logs" / "obf_preprocess_audit.json", audit_payload)

    print(
        f"OBF preprocessing complete: participants={len(participants_df)} "
